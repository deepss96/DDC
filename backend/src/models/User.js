...
class User {
  ...
  static createManagement(userData, callback) {
    const { first_name, last_name, email, username, password, role, status } = userData;

    const sql = `INSERT INTO users (first_name, last_name, email, username, password, role, status)
             VALUES (?, ?, ?, ?, ?, ?, ?)`;
    const values = [first_name, last_name, email, username, password, role || 'Field', status || 'Active'];
    db.query(sql, values, callback);
  }
